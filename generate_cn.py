...
parser.add_argument('--triplet_model_path', default= r'C:\Users\27184\Desktop\Python\huggingface\mt5-large')
parser.add_argument('--comet_model_path', default = r'C:\Users\27184\Desktop\Python\huggingface\comet-atomic-zh')
parser.add_argument('--icl_path', default = r'C:\Users\27184\Desktop\Python\MD-PCC-main\data\incontext_prompts_cn.json')
parser.add_argument('--num_beams', type=int, default=1)
parser.add_argument('--comet_num_beams', type=int, default=1)
...
incontext_prompt = json.load(open(args.icl_path, 'r', encoding='utf-8-sig'))
...
comet_model = Comet(args.comet_model_path, device=args.device, num_beams=args.comet_num_beams)
...
def generate_answer(question):
    ...
    answer_ids = triplet_model.generate(**question_ids, max_length=10, num_beams=args.num_beams, do_sample=False, early_stopping=True)
...
for split in ['train', 'val', 'test']:
    ...
    pbar_data = tqdm(total=len(data), desc='Data', position=0)
    pbar_rel = tqdm(total=len(extract_prompts_cn), desc='Relations', position=1, leave=False)
    pbar_step = tqdm(total=3, desc='Steps', position=2, leave=False)
    for string in data:
        pbar_data.update(1)
        news_triplets = []
        t5_loss = []
        for idx, rel in enumerate(list(extract_prompts_cn.keys())):
            pbar_rel.update(1)
            icl_text = ''
        ...
        pbar_step.update(1)
        pbar_rel.reset()
        gold_triplet = [[news_triplets[index][0], news_triplets[index][1], results[index][0]] for index in range(len(results))]
        t5_loss = [(num - min(t5_loss)) / (max(t5_loss) - min(t5_loss)) for num in t5_loss]
        comet_loss = [(num - min(comet_loss)) / (max(comet_loss) - min(comet_loss)) for num in comet_loss]
        for index in range(len(t5_loss)):
            if t5_loss[index] > args.t5_threshold: comet_loss[index] = 0
        score = max(comet_loss)
